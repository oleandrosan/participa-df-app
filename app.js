...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Tenta recuperar arquivos que o navegador pode ter restaurado nos inputs após reload
    verificarInputsDeArquivoAposReload();
...
// Recuperação de Arquivos pós-crash/reload
function verificarInputsDeArquivoAposReload() {
    const inputs = [document.getElementById('cameraInput'), document.getElementById('videoInput')];
    inputs.forEach(input => {
        if (input && input.files && input.files.length > 0) {
            console.log("Arquivo recuperado do input após reload!", input.files[0]);
            // Dispara manualmente o processamento
            const tipo = input.id === 'cameraInput' ? '📸 Foto' : '📹 Vídeo';
            midiaSelecionada(input, tipo);
        }
    });
}
...
async function midiaSelecionada(input, tipoIcone) {
    if (input.files[0]) {
        ...
        const novoAnexo = {
            ...
            nome: arquivo.name,
            tipo: tipoIcone,
            blob: arquivo // Salva o arquivo real no banco!
        };
