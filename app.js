...
async function restaurarEstadoTemp() {
    ...
    try {
        ...
        if (anexosSalvos && anexosSalvos.length > 0) {
            ...
            // Se houver anexos ou texto recuperado, mostra msg e abre modal
            const temTexto = salvo && JSON.parse(salvo).titulo;
            if (temTexto || listaAnexos.length > 0) {
                openModal();
                mostrarAvisoRestauracao();
            }
        } else if (salvo && JSON.parse(salvo).titulo) {
            // Se só tem texto mas não anexos
            openModal();
            mostrarAvisoRestauracao();
        }
    } catch (e) {
...
function mostrarAvisoRestauracao() {
    ...
    aviso.innerText = "🔄 Seus dados e anexos foram restaurados";
